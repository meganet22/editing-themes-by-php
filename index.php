...
$hit_sql=mysqli_query("UPDATE articles SET hit=hit+1 WHERE id='$get_id'");
} else{
$sql = "SELECT * FROM articles ORDER BY id desc LIMIT 0,3";
}
$query=mysqli_query($connection, $sql);
while($read=mysqli_fetch_assoc($query)){
    $id     = $read['id'];
...
if(isset($get_id)){
    ...
    $articles  = substr($read['articles'], 0, 450).". . .";  
    }
    //$articles  = substr(strip_tags($read['articles']), 0, 450);
    $hit    = $read['hit'];
?>
        ...
        <div class="post">
            ...
            if($admingirdi){
            echo " - <a href='yonetim.php?Act=Sil&id=$id'>Sil</a>";
            }
            ...
            <p class="post-footer align-right">
                ...
                <span class="date"><?php echo $tarih;?></span>
            </p>
        </div>
        <br/>
        <?php
